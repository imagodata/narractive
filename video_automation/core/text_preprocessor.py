...
class TextPreprocessor:
    ...
    def preprocess(self, text: str, lang: str = "fr") -> str:
        ...
        for noun, phonetic_form in sorted(proper_nouns.items(), key=lambda x: -len(x[0])):
            result = re.sub(r"\b" + re.escape(noun) + r"\b", phonetic_form, result)

        # 3. Replace acronyms with phonetic forms
        for acronym, phonetic_form in acronyms.items():
            result = re.sub(r"\b" + re.escape(acronym) + r"\b", phonetic_form, result)

        # 4. Replace spelled sigles with phonetic forms
        for sigle, phonetic_form in sigles.items():
            result = re.sub(r"\b" + re.escape(sigle) + r"\b", phonetic_form, result)
